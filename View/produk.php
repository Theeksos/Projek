...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    try {
        // Ambil nama file foto produk dulu agar filenya bisa dihapus dari folder Assets
        ...
        // Hapus foto dari server lokal jika bukan file default
...
try {
    ...
    // --- 2. LOGIKA HITUNG SUMMARY STATS (4 BAR STATUS SECARA DINAMIS) ---
    // Hitung Total Jenis Produk
    $total_produk = count($all_produk);

    // 1. Kritis: Sisa stok sudah di bawah atau pas sama dengan nilai ROP
    $stat_kritis = $koneksi->query("SELECT COUNT(*) FROM produk WHERE stok <= rop")->fetchColumn();

    // 2. Hampir Habis: Stok di atas ROP, tapi sudah mendekati (kita beri batas toleransi sampai 1.5 kali nilai ROP)
    $stat_hampir = $koneksi->query("SELECT COUNT(*) FROM produk WHERE stok > rop AND stok <= (rop * 1.5)")->fetchColumn();

    // 3. Aman: Sisa stok masih melimpah jauh di atas nilai 1.5 kali ROP
...
<head>
    ...
    <style>
        ...
        /* Style Tambahan untuk Komponen Gambar Produk */
        ...
        /* Badge Pewarnaan Status Dinamis */
        .badge-status-aman {
                        ...
                        <?php foreach ($all_produk as $row): ?>
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <!-- Path foto diarahkan ke folder Assets/images/produk/ -->
                                ...
                                <!-- Kolom 2: Nama & Keterangan (Satu Kolom) -->
                                <td>
                                ...
                                <!-- Kolom 7: Status Dinamis -->
                                <td style="text-align: center;">
                                    <?php 
                                    // Rumus penyamaan logika status berdasarkan nilai riil
                                    if ($row['stok'] <= $row['rop']) {
